...
def importar_deudores_func(path_input):
    ...
    while True:
        ...
        try:
            ...
            df= pd.read_table("{}/Inputs/DEUDORES {}.txt".format(path_input, fec_def), names= name , sep= ";", encoding='latin-1',
                              dtype={3: str}) 
...
def Reserva_SQL_Func(path_input, fec, base_1, base_2):
    
    fec_1= fec[3:7] + fec[0:2] 
    
    db= sqlite3.connect("{}/Bases/Deudores_{}.db".format(path_input, fec_1))
    
    base_1.to_sql('d4305', db, index=False, if_exists='replace')
    
    base_2.to_sql('d4306', db, index=False, if_exists='replace')
...
def main():
    ...
    path= definir_ruta_func()
    
    df, fec_def= importar_deudores_func(path)
    
    df_4305, df_4306= parsear_deudores_func(df)
    
    Reserva_SQL_Func(path, fec_def, df_4305, df_4306)
